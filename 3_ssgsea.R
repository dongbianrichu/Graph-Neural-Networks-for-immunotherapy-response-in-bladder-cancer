...
df <- read.table('IMvigor210_expression_matrix_new.txt', 
                  sep = '\t', 
                  header=T, 
                  row.names = 1)
...
geneSets <- list()
pathnames <- c()
fileName="pathway_genes_list.txt"
#fileName=args[2]
...
write.csv(ssgsea_score, file = "IMvigor210_expression_matrix.ssgsea.csv",quote = F)
